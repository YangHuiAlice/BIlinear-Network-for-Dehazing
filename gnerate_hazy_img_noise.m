...
clear;
close all;
load('NYU_filtered_A70_beta55/nyu_depth_v2_labeled.mat')
boarder = 9;
for ii = 1: size(images,4)
    ...
    % cut off boader
    I = I(boarder+1:end-boarder,boarder+1:end-boarder,:);
    tt = tt(boarder+1:end-boarder,boarder+1:end-boarder,:);
    
    A = 0.7 + 0.3 * rand(1);       % A (0.7,1.0)
    A = repmat(A,[size(I,1) size(I,2) size(I,3)]);
    
    if mod(ii,2)==0
        noise_pepper = 0.015 + 0.085 * rand(1);    % (0.015,0.1)
        A = imnoise(A,'salt & pepper',noise_pepper); % pepper       
    else
        noise_guass = 0.003 + 0.007 * rand(1);   %(0.003,0.01)
        A = imnoise(A,'gaussian',0, noise_guass);  %gauss       
    end
       
    hazy = I.*repmat(tt,[1 1 size(I,3)])+ A .* (1-repmat(tt,[1 1 size(I,3)]));
    %
    clearname = sprintf('NYU_filtered_A70_beta55/clear/%d.png', ii);
    hazyname = sprintf('NYU_filtered_A70_beta55/hazy/%d.png', ii);
    depthname = sprintf('NYU_filtered_A70_beta55/depths/%d.mat', ii);
