function [resA, resT] = vl_simplenn_grad(netA, netT, x, dzdy, resA, resT, pad_outputsize, varargin)
...
%
%   Normalization layer::
%     The normalization layer wraps VL_NNNORMALIZE(). It has fieldsx
%
...
%%
  ...
  %% for gradient A and T
  [resAGrad(n).dzdx, lossA] = vl_nnsparsegradA(resA(n).x);
  [resTGrad(n).dzdx, lossT] = vl_nnsparsegrad(resT(n).x);
  for i=n-1:-1:max(1, n-opts.backPropDepth+1)
    lA = netA.layers{i} ;
    lT = netT.layers{i} ;
    resA(i).backwardTime = tic ;
    switch lA.type
      case 'conv'
        if ~opts.accumulate
          if isfield(lA, 'weights')
            [resAGrad(i).dzdx, resAGrad(i).dzdw{1}, resAGrad(i).dzdw{2}] = ...
                vl_nnconv(resA(i).x, lA.weights{1}, lA.weights{2}, ...
                          resAGrad(i+1).dzdx, ...
                          'pad', lA.pad, 'stride', lA.stride, ...
                          cudnn{:}) ;
             %%
            [resTGrad(i).dzdx, resTGrad(i).dzdw{1}, resTGrad(i).dzdw{2}] = ...
                vl_nnconv(resT(i).x, lT.weights{1}, lT.weights{2}, ...
                          resTGrad(i+1).dzdx, ...
                          'pad', lT.pad, 'stride', lT.stride, ...
                          cudnn{:}) ;
          ...
          dzdwT = dzdwA ;
          if isfield(lA, 'weights')
            [resAGrad(i).dzdx, dzdwA{1}, dzdwA{2}] = ...
                vl_nnconv(resA(i).x, lA.weights{1}, lA.weights{2}, ...
                          resAGrad(i+1).dzdx, ...
                          'pad', lA.pad, 'stride', lA.stride, ...
                          cudnn{:}) ;
            %%
            [resTGrad(i).dzdx, dzdwT{1}, dzdwT{2}] = ...
                vl_nnconv(resT(i).x, lT.weights{1}, lT.weights{2}, ...
                          resTGrad(i+1).dzdx, ...
                          'pad', lT.pad, 'stride', lT.stride, ...
                          cudnn{:}) ;
          ...
          for j=1:2
            resAGrad(i).dzdw{j} = resAGrad(i).dzdw{j} + dzdwA{j} ;
            %%
            resTGrad(i).dzdw{j} = resTGrad(i).dzdw{j} + dzdwT{j} ;
          end
          clear dzdwA  dzdwT ;
        end
      ...
      case 'relu'
        if ~isempty(resA(i).x)
          resAGrad(i).dzdx = vl_nnrelu(resA(i).x, resAGrad(i+1).dzdx) ;
          %%
          resTGrad(i).dzdx = vl_nnrelu(resT(i).x, resTGrad(i+1).dzdx) ;
        else
          % if res(i).x is empty, it has been optimized away, so we use this
          % hack (which works only for ReLU):
          resAGrad(i).dzdx = vl_nnrelu(resA(i+1).x, resAGrad(i+1).dzdx) ;
          %%
          resTGrad(i).dzdx = vl_nnrelu(resT(i+1).x, resTGrad(i+1).dzdx) ;
        end
      case 'sigmoid'
         resA(i).dzdx = vl_nnsigmoid(resA(i).x, resAGrad(i+1).dzdx) ;
         resT(i).dzdx = vl_nnsigmoid(resT(i).x, resTGrad(i+1).dzdx) ;
    end
  end
  %=================================================%
end
...
if doder
    for i=1:n
        if strcmp(netA.layers{i}.type, 'conv') ;
            %resA(i).dzdw{1} = resA(i).dzdw{1} + resAGrad(i).dzdw{1};
            %resA(i).dzdw{2} = resA(i).dzdw{2} + resAGrad(i).dzdw{2};
            resA(i).dzdw{1} = resA(i).dzdw{1};
            resA(i).dzdw{2} = resA(i).dzdw{2};
            resT(i).dzdw{1} = resT(i).dzdw{1} + 0.1*resTGrad(i).dzdw{1};
            resT(i).dzdw{2} = resT(i).dzdw{2} + 0.1*resTGrad(i).dzdw{2};
        else
            continue;
        end
    end
