function [delta_a, delta_t, energy_val] = vl_nnhazerobustloss(A, T, labels, in, dzdy)
%
batch_num = size(in,4);
%
J = labels(:,:,1:3,:);
...
%hazyimg = T.*J + (1-T).*A;
resdual_error_tmp=(A-J);
resdual_error= resdual_error_tmp./sqrt(resdual_error_tmp.^2 + eps.^2);
%
%resdual_error2_tmp = (T_ini-gt_T);
%resdual_error2 = resdual_error2_tmp./sqrt(resdual_error2_tmp.^2 + eps.^2);

% energy_val = 0.5*sum(resdual_error(:).^2)./ batch_num + 0.5*sum(resdual_error2(:).^2)./ batch_num;
% energy_val = 0.4*sum(resdual_error(:).^2)./ batch_num + 0.6*sum(resdual_error2(:).^2)./ batch_num;
 energy_val = sum(sqrt(resdual_error_tmp(:).^2+ eps.^2))./ batch_num ;
             %0.6 * sum(sqrt(resdual_error2_tmp(:).^2+ eps.^2))./ batch_num ;
% energy_val = sum(resdual_error2(:).^2)./ batch_num;
%%
